      ...
      <Stack spacing={2}>
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          <Typography variant="subtitle2" fontWeight={600}>
            {t("clinicalChemistry.interpretation.scoreLabel")}
          </Typography>
          <Chip
            label={t("clinicalChemistry.interpretation.scoreValue", { score: result.score })}
        ...
        {result.confirmed_findings.length > 0 && (
          <Box>
            <Typography variant="subtitle2" color="success.main" gutterBottom>
              {t("clinicalChemistry.interpretation.confirmed")}
            </Typography>
        ...
        {result.missing_findings.length > 0 && (
          <Box>
            <Typography variant="subtitle2" color="text.secondary" gutterBottom>
              {t("clinicalChemistry.interpretation.missing")}
            </Typography>
            <List dense disablePadding>
        ...
        {result.incorrect_findings.length > 0 && (
          <Box>
            <Typography variant="subtitle2" color="error.main" gutterBottom>
              {t("clinicalChemistry.interpretation.incorrect")}
            </Typography>
            <List dense disablePadding>
    ...
    <Box display="flex" justifyContent="center" px={2} py={6}>
      <Paper elevation={2} sx={{ p: 4, width: "100%", maxWidth: 720 }}>
        <Stack spacing={3}>
          ...
          >
            <Box>
              <Typography variant="h5" component="h1" fontWeight={700}>
                {t("clinicalChemistry.interpretation.title")}
              </Typography>
              <Typography variant="body2" color="text.secondary">
                {t("clinicalChemistry.interpretation.subtitle")}
              </Typography>
            </Box>
          ...
          {historyError && (
            <Alert severity="error">{t("clinicalChemistry.interpretation.loadError")}</Alert>
          )}
          ...
          {submitInterpretation.isError && (
            <Alert severity="error">
              {submitInterpretation.error instanceof Error
                ...
                : t("clinicalChemistry.interpretation.submitError")}
            </Alert>
          )}

          <Stack spacing={1}>
            <Typography variant="subtitle2">
              {t("clinicalChemistry.interpretation.inputLabel")}
            </Typography>
            <TextField
              value={freeText}
              ...
              multiline
              minRows={5}
              placeholder={t("clinicalChemistry.interpretation.inputPlaceholder")}
              disabled={submitInterpretation.isPending}
              inputProps={{ "aria-label": t("clinicalChemistry.interpretation.inputLabel") }}
            />
            <Button
              variant="contained"
            ...
            >
              {submitInterpretation.isPending
                ? t("clinicalChemistry.interpretation.submitting")
                : t("clinicalChemistry.interpretation.submit")}
            </Button>
          </Stack>
          ...
          {history && history.length > 1 && (
            <>
              <Divider />
              <Typography variant="subtitle2">
                {t("clinicalChemistry.interpretation.previousAttempts", {
