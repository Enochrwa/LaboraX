...
import { InterpretationPage } from "@/features/clinicalChemistry/InterpretationPage";
...
const strongResult: InterpretationResult = {
  id: "i1",
  case_id: CASE_ID,
  submitted_text: "Serum creatinine is increased.",
  score: 80,
  confirmed_findings: [
    {
      expected_finding: "Serum creatinine is increased, confirming impaired kidney function",
      matched_statement: "Serum creatinine is increased.",
      similarity: 0.9,
      topic: "renal_function",
      explanation:
        "Creatinine is cleared almost entirely by the kidneys, so a rise is a specific marker of declining renal function.",
    },
  ],
  missing_findings: [],
  incorrect_findings: [],
...
const weakResult: InterpretationResult = {
  id: "i2",
  case_id: CASE_ID,
  submitted_text: "Sodium is increased.",
  score: 10,
  confirmed_findings: [],
  missing_findings: [
    {
  ...
  incorrect_findings: [
    {
      statement: "Sodium is increased.",
      reason: "Expected sodium to be decreased.",
      topic: "electrolyte_balance",
    },
  ],
    ...
    <Provider store={store}>
      <QueryClientProvider client={queryClient}>
        <I18nextProvider i18n={i18n}>
          <MemoryRouter initialEntries={[`/chemistry/case/${CASE_ID}/interpretation`]}>
            <Routes>
              <Route
...
describe("InterpretationPage", () => {
  it("submits free text and displays the score and feedback", async () => {
    ...
    await user.type(input, "Serum creatinine is increased.");
  ...
  it("shows missing and incorrect findings for a weak submission", async () => {
    ...
    const input = await screen.findByLabelText("Your interpretation");
    await user.type(input, "Sodium is increased.");
    await user.click(screen.getByRole("button", { name: "Submit interpretation" }));

    expect(await screen.findByText("10 / 100")).toBeInTheDocument();
    expect(
      screen.getByText("Serum creatinine is increased, confirming impaired kidney function"),
    ).toBeInTheDocument();
    expect(screen.getByText("Sodium is increased.")).toBeInTheDocument();
    expect(screen.getByText(/Expected sodium to be decreased\./)).toBeInTheDocument();
  });
