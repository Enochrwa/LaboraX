    ...
    <Paper variant="outlined" sx={{ p: 2 }}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={1}>
        ...
        {flag && (
          <Chip
            size="small"
            label={t(`hematology.testOrdering.flags.${flag}`, { defaultValue: flag })}
          ...
          {Object.entries(values).map(([parameter, value]) => (
            <Stack key={parameter} direction="row" justifyContent="space-between">
              <Typography variant="body2" color="text.secondary">
                {t(`hematology.testOrdering.parameterLabels.${parameter}`, {
              ...
              <Stack direction="row" spacing={1} alignItems="center">
                ...
                <Chip
                  size="small"
                  variant="outlined"
                  label={t(`hematology.testOrdering.flags.${flags[parameter]}`, {
      ...
      <Paper elevation={2} sx={{ p: 4, width: "100%", maxWidth: 720 }}>
        <Stack spacing={3}>
          ...
          >
            <Box>
              <Typography variant="h5" component="h1" fontWeight={700}>
                {t("hematology.testOrdering.title")}
              </Typography>
              <Typography variant="body2" color="text.secondary">
                {t("hematology.testOrdering.subtitle")}
              </Typography>
            </Box>
          ...
          {(catalogError || resultsError) && (
            <Alert severity="error">{t("hematology.testOrdering.loadError")}</Alert>
          )}
          ...
          {orderTests.isError && (
            <Alert severity="error">
              {orderTests.error instanceof Error
                ...
                : t("hematology.testOrdering.orderError")}
            </Alert>
          )}
          ...
          {catalog && (
            <Stack spacing={1}>
              <Typography variant="subtitle2">
                {t("hematology.testOrdering.selectTests")}
              </Typography>
              <Stack>
                {catalog.map((test) => {
                        ...
                        <Stack direction="row" spacing={1} alignItems="center">
                          ...
                          {alreadyOrdered && (
                            <Chip
                              size="small"
                              variant="outlined"
                              label={t("hematology.testOrdering.alreadyOrdered")}
                            />
                          )}
                        </Stack>
                      }
              ...
              >
                {orderTests.isPending
                  ? t("hematology.testOrdering.ordering")
                  : t("hematology.testOrdering.orderSelected")}
              </Button>
            </Stack>
          )}
          ...
          <Stack spacing={2}>
            <Stack direction="row" justifyContent="space-between" alignItems="center">
              <Typography variant="subtitle2">{t("hematology.testOrdering.results")}</Typography>
              <Chip
                size="small"
                label={t("hematology.testOrdering.totalPenalty", { penalty: totalPenalty })}
            ...
            {caseResults && caseResults.results.length === 0 && (
              <Typography variant="body2" color="text.secondary">
                {t("hematology.testOrdering.noResultsYet")}
              </Typography>
            )}
